<x-layout title="Sticker Edit">
    <form action="{{ route('stickers.update', $sticker->uuid) }}" method="post" autocomplete="off"
        ...
        <div class="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
            <div class="trezo-card-content">
                ...
                <x-form.input label="Freelance Price" id="freelancePrice" name="freelance_price" :value="$sticker->freelance_price"
                    :required="true" />
                <x-form.select label="Type" id="type" name="type" :required="true">
                    <option value="car" @if ($sticker->type == 'car') selected @endif>Car</option>
                    <option value="motorcycle" @if ($sticker->type == 'motorcycle') selected @endif>Motorcycle</option>
                </x-form.select>
                <x-form.select label="Brand" id="brand" name="brand">
                    <option value="honda" @if ($sticker->brand == 'honda') selected @endif>Honda</option>
                    <option value="yamaha" @if ($sticker->brand == 'yamaha') selected @endif>Yamaha</option>
                    <option value="kawasaki" @if ($sticker->brand == 'kawasaki') selected @endif>Kawasaki</option>
                    <option value="suzuki" @if ($sticker->brand == 'suzuki') selected @endif>Suzuki</option>
                </x-form.select>
                <x-form.file label="Change Sticker Image" id="image" name="img_file" class="sm:col-span-2" />
            </div>
        </div>
